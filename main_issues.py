...
import nltk #stopwards for resumepy
import pathlib #get path of file
...
def uploadResume(file):
    ...
    nm = str(secure_filename(file.filename))
    path2 = os.path.abspath(nm)
    print(path2)
    if file: 
        try:
            nltk.download('stopwords') #stopwards for resumepy
            bucket = storage.bucket()
            #file is just an object from request.files e.g. file = request.files['myFile']
            doc = Document()
            with open(path2,'r') as file:
                doc.add_paragraph(file.read()) #refers to local "file"
            doc.save(nm)
            data = ResumeParser(nm).get_extracted_data()
            f = open(nm,"a")
            f.write(data)
            f.close()
            #doc.save(os.path.join(os.path.abspath(os.path.dirname(__file__)), app.config['UPLOAD_FOLDER'], secure_filename(file.filename))) # Save file to resumes folder locally 
